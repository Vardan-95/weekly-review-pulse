"""CLI entrypoint — Architecture.md §3/§10: `pulse run`, `pulse backfill`,
`pulse status`, now wired to the real orchestrator (pulse/orchestrator/run.py)
instead of Phase 1's stub.
"""
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--env", default="dev", help="Environment name from environments.yaml (default: dev)")
...
def _load_config() -> tuple[dict, dict]:
    products = p.load_products(_PRODUCTS_PATH)
    environments = p.load_environments(_ENVIRONMENTS_PATH)
    return products, environments


def _resolve_week(week_arg: str | None) -> str:
    if week_arg is None:
        return p.current_iso_week()
    year, week = p.parse_iso_week(week_arg)
    return p.format_iso_week(year, week)


def cmd_status(args: argparse.Namespace) -> int:
    products, _ = _load_config()
    p.get_product(products, args.product)  # validates the product exists
    iso_week = _resolve_week(args.week)

    with p.RunLedger(args.db) as ledger:
    ...
    print(f"  doc:   status={record.doc_status} named_range={record.doc_named_range} link={record.doc_deep_link}")
    print(f"  email: status={record.email_status} message_id={record.email_message_id} mode={record.email_mode}")
...
def _run_or_backfill(args: argparse.Namespace, iso_week: str) -> int:
    ...
    if summary.status == "SUCCEEDED":
        print(
            f"  doc={summary.doc_status} email={summary.email_status} "
            f"themes={summary.themes_included} quotes={summary.quotes_validated} "
            f"tokens={summary.tokens_used} cost_usd={summary.cost_usd:.4f} "
...
def cmd_backfill(args: argparse.Namespace) -> int:
    year, week = p.parse_iso_week(args.week)
    iso_week = p.format_iso_week(year, week)
    return _run_or_backfill(args, iso_week)
...
def main(argv: list[str] | None = None) -> int:
    ...
    except (p.ConfigValidationError, p.InvalidIsoWeekError) as exc:
        print(f"error: {exc}", file=sys.stderr)
        return 1
